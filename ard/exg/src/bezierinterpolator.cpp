...
ard::BezierController& ard::BezierController::instance()
{
    static BezierController b;
    static bool firstCall = true;
    if (firstCall) {
        const int pointsNumber = 4;
        int middleKnotNumber = pointsNumber - 4;
        b.knotVector.clear();
        for (int counter = 0; counter < 4; ++counter)
            b.knotVector.push_back(0.0);
        for (int counter = 1; counter <= middleKnotNumber; ++counter)
            b.knotVector.push_back(1.0 / (middleKnotNumber + 1) * counter);
        for (int counter = 0; counter < 4; ++counter)
            b.knotVector.push_back(1.0);
    }
    return b;
};