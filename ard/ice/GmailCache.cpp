...
void mail_cache::ThreadData::add_msg(msg_ptr m) 
{
    ...
    if (i != m_mmap.end()) {
        qWarning() << "duplicate message in thread" << id() << m->id();
    }
#endif
...
bool mail_cache::ThreadData::hasAllLabels(uint64_t data)const
{
    bool rv = true;
    if (data != 0) {
        uint64_t f = data & m_thread_labels;
        rv = (data == f);
    }
    return rv;
};
...
QString mail_cache::QueryData::format_qhash(QString qstr, QString lblid, bool as_filter)
{
    QString rv = qstr + ":" + lblid + ":" + (as_filter ? "1" : "0");
  return rv;
};
...
bool mail_cache::GMailSQLiteStorage::ensureMailTables() 
{
    ...
    QString sql_messages = QString("CREATE TABLE IF NOT EXISTS %1gmail_msg(acc_id INTEGER NOT NULL, thread_id TEXT NOT NULL, msg_id TEXT NOT NULL, msg_from TEXT, "
        ...
        "msg_html TEXT, internal_date INTEGER, msg_state INTEGER, msg_labels INTEGER, msg_references TEXT)").arg(m_metaPrefix);
    ...
    /// queries ///
    QString sql_q = QString("CREATE TABLE IF NOT EXISTS %1gmail_q(q_id INTEGER PRIMARY KEY, acc_id INTEGER NOT NULL, filter_mask INTEGER, q_query TEXT, labelid TEXT, backend_token TEXT)")
        .arg(m_metaPrefix);
    if (!execQuery(sql_q))
        return false;
...
std::vector<mail_cache::SysLabel> mail_cache::mask2reservedSysLabel(uint64_t composite_mask)
{
    std::vector<mail_cache::SysLabel> rv;
    static uint64_t theone = 1;
    int idx = (int)SysLabel::IMPORTANT;
    int Max = (int)SysLabel::CATEGORY_PROMOTIONS;
    for (; idx <= Max; idx++) {
        uint64_t label_mask = (theone << idx);
        if (composite_mask & label_mask) {
            SysLabel lbl = (SysLabel)idx;
            rv.push_back(lbl);
        }
    }
...
QSqlQuery* mail_cache::GMailSQLiteStorage::doPrepareQuery(std::unique_ptr<QSqlQuery>& q, QString sql)const
{
    if (!q)
    {
        qWarning() << "ERROR. Expected internal query";
...
QSqlQuery* mail_cache::GMailSQLiteStorage::prepareQuery(QString sql)const
{
    return doPrepareQuery(m_query, sql);
};

QSqlQuery* mail_cache::GMailSQLiteStorage::prepareContactQuery(QString sql)const
{
    return doPrepareQuery(m_contact_query, sql);
};

QSqlQuery* mail_cache::GMailSQLiteStorage::selectQuery(QString sql)const
{
    QSqlQuery* q = prepareQuery(sql);
    if (!q)return nullptr;
    if(!q->exec(sql))
...
mail_cache::msg_ptr mail_cache::GMailSQLiteStorage::findMessage(QString msg_id) 
{
    mail_cache::msg_ptr rv;
    if (!m_msg_cache) {
        return nullptr;
    }
    rv = m_msg_cache->mem_object(msg_id);
    return rv;
};

googleQt::mail_cache::thread_list mail_cache::GMailSQLiteStorage::loadThreadsByIdsFromDb(const std::vector<QString>& thread_ids)
{
    return m_tstorage->loadThreadsByIdsFromDb(thread_ids);
};

googleQt::mail_cache::msg_list mail_cache::GMailSQLiteStorage::loadMessagesByIdsFromDb(const std::vector<QString>& msg2load)
{
    msg_list rv;
    ...
    std::function<bool(const STRING_LIST& lst)> loadList = [&](const STRING_LIST& lst) -> bool
    {
        ...
        QString sql = QString("SELECT t.thread_id, t.history_id, t.messages_count, t.snippet, t.filter_mask, "
            ...
            "WHERE m.acc_id = t.acc_id AND m.thread_id = t.thread_id AND m.acc_id = %2 AND msg_id IN(%3)")
            .arg(m_metaPrefix)
            .arg(m_accId)
            .arg(comma_ids);
        ...
        while (q->next())
        {
            ...
            if (t) {
                googleQt::mail_cache::msg_ptr m = mc->mem_object(msg_id);
                if (m) {
                    located_msg++;
                }
                else {
...
#ifdef API_QT_AUTOTEST
        QString sdbg = QString("L/ +t=%1 !t=%2 +m=%3 !m=%4")
            .arg(loaded_threads)
            .arg(located_threads)
            .arg(loaded_msg)
            .arg(located_msg);
    ...
    if (!msg2load.empty()) 
    {
        if (!chunk_list_execution(msg2load, loadList))
        {
            qWarning() << "ERROR. Failed to load list of messages.";
        }
    }
...
mail_cache::msg_ptr mail_cache::GMessagesStorage::loadMessageFromDb(thread_ptr t2, QSqlQuery* q, int fidx)
{
    if (!t2) {
        qWarning() << "ERROR. Expected valid thread " << q->value(1).toString();
        return nullptr;
    }
    ...
    /*
    QString thread_id = q->value(13 + fidx).toString();
    if (thread_id.isEmpty())
    {
...
googleQt::mail_cache::thread_list mail_cache::GThreadsStorage::loadThreadsByIdsFromDb(const std::vector<QString>& thread_ids)
{
    thread_list rv;
    ...
    std::function<googleQt::mail_cache::thread_list(QString)> process_list = [&](QString ids_str)
    {
        ...
        if (load_messages_for_threads && loaded_threads.size() > 0) {
            QString loaded_threads_id_str = slist2str_decorated(loaded_threads.begin(), loaded_threads.end());
...
#define BATCH_SIZE 400
    ...
    for (const auto& id : thread_ids) {
        ...
        {
            QString ids_str = slist2str_decorated(batch_list.begin(), batch_list.end());
...
{
    QString sql = QString("SELECT q_id, q_query, labelid, backend_token, filter_mask FROM %1gmail_q "
        "WHERE acc_id=%2")
    ...
    while (q->next())
    {
        ...
        auto fmask = q->value(4).toULongLong();
...
mail_cache::query_ptr mail_cache::GQueryStorage::ensure_q(QString q_str, QString labelid, bool as_filter)
{
    ...
    auto fmask = 0;
    if (as_filter) {
        fmask = nextFilterMask();
    }
    QString sql = QString("INSERT INTO %1gmail_q(acc_id, q_query, labelid, filter_mask) VALUES(%2,?,?,?)")
    ...
    if (q->exec()) {
        ...
        auto qd = std::shared_ptr<QueryData>(new QueryData(q_id, q_str, labelid, "", fmask));
...
mail_cache::query_ptr mail_cache::GQueryStorage::lookup_q(QString q_str, QString labelid, bool as_filter)
{
    auto i = m_qmap.find(QueryData::format_qhash(q_str, labelid, as_filter));
    if (i == m_qmap.end()) {
        return nullptr;
...
uint64_t mail_cache::GQueryStorage::nextFilterMask()const 
{
    static uint64_t theone = 1;
    for (int i = 0; i < 64; i++) {
        uint64_t check_mask = (theone << i);
        if (!(m_all_filters_mask & check_mask))
            return check_mask;
    }
    qWarning() << "no more filters, hit 64 limit";
    return 0;
};
